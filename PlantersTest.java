...
public class PlantersTest {
	...
	public static void main(String[] args) {
		...
		for(int k = 0; k < n; k++)
			System.out.print(availablePlanters[k] + " ");
		System.out.println();
		for(int k = 0; k < m; k++)
			System.out.print(extraPlanters[k] + " ");
		System.out.println();
		...
		for(i = 0 ; i < n; i++) {
			...
			if(i != n-1 && availablePlanters[i] < availablePlanters[i+1]) {
				...
				while(j < m) {
					if(extraPlanters[j] > availablePlanters[i] && extraPlanters[j] < availablePlanters[i+1]) {
						availablePlanters[i] = -1;
						j++;
						flag1 = 1;
						break;
					}
					if(extraPlanters[j] > availablePlanters[i]) {
						break;
					}
					j++;
				}
